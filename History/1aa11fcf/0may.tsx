...
export default function SetComponentsDestaque({
  ...
  const [fragFem, setFragFem] = useState(false)
   ...
   lg:w-[40dvh] "
    >
      <section className="flex flex-col w-full  px-5">
        <h1 className="text-2xl font-bold mb-4 text-gray-200" ref={titleRef}>Fragrâncias em Destaque</h1>
        <div className="flex  w-full h-full  p-2 gap-5">
          <button
            className={`py-2 text-sm
            ${fragMasc ? 'text-gray-100 border-b-2 border-amber-400' : 'text-gray-400 border-b-2 border-transparent'}
          ...
          <button
            className={`py-2 text-sm
                  ${fragFem ? 'text-gray-100 border-b-2 border-amber-400' : 'text-gray-400 border-b-2 border-transparent'}
                  transition-colors duration-300`}
            onClick={() => {
              setFragFem(!fragFem)
              setFragMasc(false)
