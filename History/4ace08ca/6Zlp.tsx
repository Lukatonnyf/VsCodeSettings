...
export default function ListaDados({ className }: { className: string; }) {
  ...
  return (
    <div className={`${className} flex h-full w-full  justify-center bg-red-4`}>
      <ul className="grid grid-cols-1 md:grid-cols-3 gap-5 p-5 w-full">
        {
          dados.length > 0 ? (
            dados.map((item) => (
              <li
                key={item._id}
                className="flex flex-col justify-between gap-2 md:max-w-[25vw] w-full min-h-[35vh] border border-bordercomponents p-5 rounded-xl"
              >
            ...
            <li className="col-span-full w-full h-[60vh] flex flex-col justify-center items-center border border-white rounded-xl text-white text-center gap-4">
              <p>Ainda não possuo Feedbacks :(</p>
              <Button
                onClick={() =>
                  window.open('https://feedbacks-page-two.vercel.app/', "_blank")
                }
              >
