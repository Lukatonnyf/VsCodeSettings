...
export default function ListaDados({ className }: { className: string; }) {
  ...
  const scrollContainerRef.current = useRef<(HTMLLIElement | null)[]>([])
      ...
      if (card) {
        const anim = gsap.fromTo(
          ...
          { opacity: 0, y: 50 }, // Estado inicial
          {
            ...
            scrollTrigger: {
              trigger: scrollContainerRef.current, // O card será o trigger para a animação
              start: "center center",
              end: "top 30%",
  ...
  return (
    <div
      ...
      className={`${className} flex h-full w-full  justify-center bg-red-4`}>
