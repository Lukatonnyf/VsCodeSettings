          ...
          {ArrayFragances.length > 0 ? (
            ArrayFragances.map((frag, index) => (
              ...
              >
                ...
                <article className='flex flex-col text-start gap-1'>
                  <p className="flex flex-row text-lg gap-2md:text-2xl lg:text-xl">
                    <span className={`${geistMono.variable} text-white`}>{frag.title}</span>
                  </p>

                  <p className="flex flex-col font-semibold gap-1 lg:text-sm">
                    <span
                      className={`${sansita.variable} font-light text-md break-words whitespace-pre-wrap text-gray-200`}>
