...
export default function ActivityForm() {
  ...
  const [showForm, setShowForm] = useState(false); // Para mostrar ou esconder o formulário
  const [formData, setFormData] = useState({ nameActivity: '', hour: '' }); // Para armazenar os dados do formulário
  const [editIndex, setEditIndex] = useState<number | null>(null);

  const handleEdit = (index: number) => {
    const activity = activities[index]; // Pega a atividade na posição 'index'
    setFormData({ nameActivity: activity.nameActivity, hour: activity.hour }); // Preenche os dados no formulário
    setEditIndex(index); // Marca que estamos editando essa atividade
  ...
  return (
    <div className="flex flex-col w-full h-full  relative lg:flex-row lg:p-0 overflow-x-hidden overflow-y-hidden">
      ...
      <aside className="w-full py-2 px-2 lg:px-0 lg:pr-2" >
        <div ref={cardAtividades} className="w-full h-full pb-10 p-5 bg-card rounded-2xl  lg:h-[40dvh] lg:rounded-sm">
          {activities.length > 0 ? (
            <>
