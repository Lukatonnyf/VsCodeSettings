...
export default function VsMyHobbys(){
 ...
 const irCertificado = (url: string) => {
    ...
    return(
      <div className="w-full h-full bg-purple p-2">
        <div>
            <h1>certificados</h1>

        </div>
          <div className="w-full max-h-full  bg-blue p-3 pt-4 flex flex-col  justify-center items-center  gap-5 md:flex-row md:flex-wrap"> 
          {
            certificados.map((certificadosCursos) =>
            <div key={certificadosCursos.id} className="max-w-[35dvh] w-full max-h-full h-full flex flex-col justify-between  items-start  p-2 pb-5 border border-gray-500 bg-gray-600 gap-2 rounded-2xl sm:w-[35dvh] sm:h-[30dvh] transition  sm:hover:scale-105">
            <Image 
             ...
             <button type="button" className="mt-1 py-2 p-2 w-[20dvh] text-gray-100 bg-btn rounded-2xl  font-sans text-sm transition hover:bg-btn-hover" onClick={() => irCertificado(certificadosCursos.linkCertificado)}>Ir para o projeto</button>
            </div>
          </div>  
            )
          }
