...
export default function AboutAbner() {
  ...
  useEffect(() => {
    descriptionAbnerRef.current.forEach((el, index) => {
      if (el) {
        gsap.fromTo(
          el,
          { opacity: 0, y: 100 },
  ...
  return (
    <div className="flex flex-col justify-center items-center
     w-full min-h-100 text-white p-5    bg-gradient-to-t from-black to-[#121212]">
      ...
      >
        ...
        <div className='lg:ml-5 mb-7 md:mb-0 w-full h-full flex flex-col gap-5  justify-center items-center
         p-5'>

          {/* primeiro card */}
          <div
            ref={(el) => { descriptionAbnerRef.current[0] = el }}
          ...
          {/* segundo card  */}
          <div
            ref={(el) => { descriptionAbnerRef.current[1] = el }}
            className='border border-borderbd rounded-2xl p-5 shadow-shadowcard'>
            <header className='flex flex-col justify-start   text-start '>
              <p className='flex justify-center items-center w-15 h-15 bg-yellow-600  rounded-full shadow-shdamber '>
