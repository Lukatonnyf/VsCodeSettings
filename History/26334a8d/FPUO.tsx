...
export default function Header() {
  ...
  const fraqDestaque = useRef<HTMLUListElement>(null);
  const feedBack = useRef<HTMLLIElement>(null);
  useLayoutEffect(() => {
    if (isOpen) {
      gsap.fromTo(
  ...
  const handleScroll = (id: string | number) => {
    let targetElement;
    if (id === 'fraqDestaque') {
      targetElement = fraqDestaque;  // Supondo que fraqDestaque seja um elemento de referência
    } else if (id === 'feedBack') {
      targetElement = feedBack;  // Supondo que feedBack seja outro elemento de referência
    }
        ...
        {!isOpen ? (
          ...
          justify-center items-center ">
            {ArrayItemsHeader.map(item => (
              <li
                onClick={() => { setSelected(item.id); handleScroll(item.id); }}
