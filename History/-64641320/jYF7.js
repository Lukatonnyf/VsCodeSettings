const input = document.getElementById('number')
const button = document.getElementById('button')


function For(){

  let num = input.value
  try{
    throw 'erro gerado'
    for(num; num <= 10; num++){
      console.log(num)
    }
  }catch(e){
  console.log(e)
}
}

button.onclick = (e) => {
    e.preventDefault()
  For()

}



const names = ['tonny', 'é', 'lindo', 'demais']

  for(let i = 0; i < names.length; i++){
    console.log(names[i])
  }

// for OF
// Melhor opção para ser utilizada Caso não precise do índice
/** @FOR OF
 * explicação
  Criamos uma variável interna de índices dentro do for,
   para percorrer a quantidade total de íncides de todo
   o array e retorna o índice CORRENTE para a váriavel

   EXEMPLO:

  for(const name of names){
    console.log(name)
  }

   const name === ( A VARIÁVEL QUE PERCORRE OS ÍNDICES)
   OF === (ÍNDICA O ARRAY QUE DEVE PERCORRER)
   name === (LOGO APÓS, A VARIÁVEL RECEBE O ÍNDICE CORRENTE)
 */



// For In, Retorna sempre a Propiedade do OBJETO
/** @FOR In
 *                        Explicação:
  Ele itera sobre um OBJETO, assim como no array,
  porém, por padrão, ele sempre vai retorna a PROPIEDADE
  do objeto

 *               COMO ACESSAR O VALOR DA PROPIEDADE?
  Para acessar o valor da propiedade, deve-se colocar o
  seguinte código:
  Exemplo: ${propiedade[prop]}

 *                      EXEMPLO PRATICO
 const person = {
  name: 'tonny',
  age: 17
 }

  for(const prop in person){
    console.log(`prop: ${prop} - ${person[prop]}`)
  }



   const prop ==> ( A VARIÁVEL QUE PERCORRE OS ÍNDICES)
   In ==> (ÍNDICA O OBJETO QUE DEVE PERCORRER)
   prop ==> (LOGO APÓS, A VARIÁVEL RECEBE O ÍNDICE CORRENTE
   E A PROPIEDADE E/OU VALOR.)
 */

