        ...
        <div className="w-full h-full flex  justify-center items-center p-5  flex-col  gap-5 md:flex-row md:flex-wrap"> 
           
            {
                MyProjects.map((project) => (
                    <div key={project.id} className="max-w-full w-full max-h-full h-full flex flex-col justify-between  items-start  p-2 pb-5 border border-gray-500 bg-gray-600 gap-2 rounded-2xl sm:w-[40dvh] sm:h-[42dvh] ">
                    {project.img && (
                      <Image 
                        src={project.img} 
                        alt={project.name} 
