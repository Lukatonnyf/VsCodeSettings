  ...
  return (
    <div className="w-full h-full flex  justify-center items-center p-5  flex-col  gap-5 md:flex-row md:flex-wrap">
      {MyProjects.map(project => (
        <div
          key={project.id}
          className="max-w-full w-full max-h-full h-full flex flex-col justify-between  items-start  p-2 pb-5 border border-gray-500 bg-gray-600 gap-2 rounded-2xl sm:w-[40dvh] sm:h-[42dvh] transition  sm:hover:scale-105"
        >
          {project.img && (
            <Image
              src={project.img}
          ...
          <button
            type="button"
            className="mt-1 py-2 p-2 w-[20dvh] text-gray-100 bg-btn rounded-2xl  font-sans text-sm transition hover:bg-btn-hover"
