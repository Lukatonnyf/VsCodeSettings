...
export default function FormFeedback({ onSubmitAction, show, setShow }: FeedbackFormProps) {
  ...
  const handleClickOutside = (e: MouseEvent) => {
    if (cardRef.current && !cardRef.current.contains(e.target as Node)) {
      closeCard(); // Fecha o card se o clique for fora
    }
  };
  ...
  useEffect(() => {
    ...
    // Cleanup: remove o event listener quando o componente for desmontado ou showCard mudar
    return () => {
      document.removeEventListener("click", handleClickOutside);
    };
  }, [showCard]);
  ...
  return (
    <form onSubmit={handleSubmit}
     ...
     w-2/3 h-[50dvh] top-50 z-10 p-5 rounded-xl lg:w-1/2
     text-gray-300 gap-2
    bg-background border-borderbd border"  >
      <label className="flex justify-between">Descreva Sua experiencia
        <span className="text-gray-100" onClick={() => setShow(false)}>X</span>
      </label>
