...
import { PageParams } from '../profile/[id]/page';



export default function HomePage({ className, params }: { className: string; params: { id: string } }) {

  const goToLoginPage = async (params: { id: string }) => {
    const { id } = params;
    const db = await connectToDatabase(process.env.MONGODB_URI!);
    const user = await db.collection("users").findOne({ _id: new ObjectId(id) });

    if (user) {
      window.location.href = '/cadastrar-negocio';
    } else {
