...
export default function ImageCarousel() {
  ...
  useEffect(() => {
    ...
    // Distribui os logos lado a lado
    gsap.set(boxes, {
      xPercent: (i) => i * 100,
    });
  ...
  const fullList = [...logos, ...logos]; // duplicado para efeito contínuo

  return (
    <div className="overflow-hidden bg-black py-8 w-full">
      <div className="flex w-max gap-12" ref={wrapperRef}>
        {fullList.map((src, idx) => (
          <div
            key={idx}
            className="logo-slide flex-shrink-0"
          >
            <img
              src={src}
              alt={`Logo ${idx}`}
