    ...
    >
      ...
      >
        {displayedItems.map((item, index) => (
          <div
            key={`${item.id}-${index}`}
            className="flex-shrink-0 flex justify-center items-center gap-2 "
            style={{ width: `${LOGO_WIDTH}px`, marginRight: `${LOGO_GAP}px` }}
          >
            <p
              className={`w-${LOGO_WIDTH} h-5 w-auto object-contain text-shadow-degradetext-to`}
            >{item.imageUrl}</p>
            {/* <Image
              src={item.imageUrl}
              alt={item.altText}
              ...
              draggable="false"
            /> */}
            <p className='text-black text-sm font-bold '>{item.textLenaguage}</p>
          </div>
        ))}
      </div>
    </div>
